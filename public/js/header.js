...
function date(date)
{
	...
	var month = {"01": 'Январь', "02": 'Февраль', "03": 'Март', "04": 'Апрель', "05": 'Май', "06": 'Июнь', "07": 'Июль', "08": 'Август', "09": 'Сентябрь', "10": 'Октябрь', "11": 'Ноябрь', "12": 'Декабрь'};
...
function callback_header(data){
    ...
    else if (data[0] == 1)
    {
        ...
        if (data[1].length == '0')
        {
            ...
            notif.innerHTML ="<p style='font-size:18px;color: grey;font-style: italic;text-align:center;width:100%'>У вас нет уведомлений</p>";
        ...
        else 
        {
            for(i = 0 ; i < data[1].length ; i++)
            {
                ...
                if(data[1][i].text == null)
                    ...
                    var item = 'прокомментаровал';
                notif.innerHTML ="<img id='notif-logo' src='"+data[1][i].profile+"'><p><b>"+data[1][i].login+"</b> a "+item+" твоё фото</br><span id='notif-date'>"+date(data[1][i].date)+"</span></p><img id='notif-img' src='"+data[1][i].img+"'>"; 
                notif_menu.appendChild(notif);
            }
            if (data[2] != '0')
            {
